...
task :test_app do
  ...
  class SpreeVendorsTestAppGenerator < Spree::Generators::TestAppGenerator
    def tweak_gemfile
      append_file 'Gemfile' do
<<-gems
gem 'spree_core', :path => '#{File.join(SPREE_PATH, 'core')}'
gem 'spree_auth', :path => '#{File.join(SPREE_PATH, 'auth')}'
gem 'spree_vendors', :path => '#{File.dirname(__FILE__)}'
gems
      end
    end
    ...
    def install_gems
      inside "test_app" do
        ...
        run 'rake spree_vendors:install'
      end
    end
  ...
  SpreeVendorsTestAppGenerator.start
end
